package business
{
	...
	import util.Server;
	import util.Helper;
	public class AbiTestDeltegate
	{
		public function AbiTestDeltegate(responder:IFSCommand)
		{
		...
		function get Ws():WebService
		{
			...
			{
				ws=new WebService(); 
				ws.loadWSDL(Server.getWsUrl("能力考评WS"));
		...
		private function generateWordResultHandler(event:ResultEvent):void
		{
			...
			{
				navigateToURL(new URLRequest(Server.getWordUrl(AbiTestLocator.getInstance().filename)));
			}
		}
		...
		{
			var locObj:AbiTestLocator=AbiTestLocator.getInstance();
			Ws.Insert.send(locObj.voObj.ID,locObj.voObj.PersonID,locObj.voObj.Name,locObj.voObj.Office,locObj.voObj.ZC,locObj.voObj.TestType,locObj.voObj.TestTime,
				locObj.voObj.TestText,locObj.voObj.TestJG,locObj.voObj.TestPJ,locObj.voObj.TestPerson,locObj.voObj.KPTime,locObj.voObj.Remack);
			PsbLocator.getInstance().npending=false;
		}		
		public function update():void
		{
			var locObj:AbiTestLocator=AbiTestLocator.getInstance();
			Ws.Update.send(locObj.voObj.ID,locObj.voObj.Name,locObj.voObj.Office,locObj.voObj.ZC,locObj.voObj.TestType,locObj.voObj.TestTime,
				locObj.voObj.TestText,locObj.voObj.TestJG,locObj.voObj.TestPJ,locObj.voObj.TestPerson,locObj.voObj.KPTime,locObj.voObj.Remack);
			PsbLocator.getInstance().npending=false;
		}		
		public function deleteFunc():void
		{
			Ws.Delete.send(AbiTestLocator.getInstance().voObj.ID);
			PsbLocator.getInstance().npending=false;
		}
