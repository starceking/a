...
namespace DBOper
{
    ...
    /// 电泳
    /// </summary>
    public static class IdEp
    {
        public const string TABLE = "id_ep";

        public static async Task<string> Insert(string number, string name, int user_id, string shelf_type,
            DateTime id_day, string machine, string board, string sheet, string sampleIds)
        {
            ...
            using (MySqlConnection dbConnection = await DBHelper.GetOpenCon())
            {
                using (MySqlTransaction trans = dbConnection.BeginTransaction())
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(sheet)) dict.Add("sheet", sheet);
                    ...
                    foreach (string sid in sampleIds.Split(','))
                    {
                        ...
                        if (ulong.TryParse(sid, out sample_id))
                            await IdEpSample.Insert(newId, sample_id, user_id, dbConnection, trans);
                    }
        ...
        public static async Task<string> Delete(ulong id, int user_id)
        {
            ...
            using (MySqlConnection dbConnection = await DBHelper.GetOpenCon())
            {
                using (MySqlTransaction trans = dbConnection.BeginTransaction())
                {
                    ...
                    await IdEpSample.Delete(0, id, dbConnection, trans);
        ...
        public static async Task<IEnumerable<IdEpModel>> GetList(string number, string user_id, string id_days, string id_daye,
            string page_size, string page_index)
        {
            ...
            return await DBHelper.GetList<IdEpModel, long>(TABLE, "*", "id", fdict, "and",
        ...
        public static async Task<IdEpModel> GetOne(ulong id)
        {
            return await DBHelper.GetOne<IdEpModel>(id.ToString());
        }
        public static async Task DeleteRedis(ulong id)
        {
            if (id <= 0) return;
            await Redis.Delete<IdEpModel>(id.ToString());
        }
    }
}
