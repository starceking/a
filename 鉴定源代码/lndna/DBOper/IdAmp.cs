...
namespace DBOper
{
    ...
    /// 扩增
    /// </summary>
    public static class IdAmp
    {
        public const string TABLE = "id_amp";

        public static async Task<string> Insert(string number, string name, int user_id, string shelf_type,
            DateTime id_day, string method, string machine, string kit, int amount, string batch,
            int circle, string board, string sampleIds)
        {
            ...
            using (MySqlConnection dbConnection = await DBHelper.GetOpenCon())
            {
                using (MySqlTransaction trans = dbConnection.BeginTransaction())
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(method)) dict.Add("method", method);
                    if (!string.IsNullOrWhiteSpace(machine)) dict.Add("machine", machine);
                    if (!string.IsNullOrWhiteSpace(kit)) dict.Add("kit", kit);
                    dict.Add("amount", amount.ToString());
                    if (!string.IsNullOrWhiteSpace(batch)) dict.Add("batch", batch);
                    dict.Add("circle", circle.ToString());
                    if (!string.IsNullOrWhiteSpace(board)) dict.Add("board", board);
                    ...
                    foreach (string sid in sampleIds.Split(','))
                    {
                        ...
                        if (ulong.TryParse(sid, out sample_id))
                            await IdAmpSample.Insert(newId, sample_id, user_id, dbConnection, trans);
                    }
        ...
        public static async Task<string> Delete(ulong id, int user_id)
        {
            ...
            using (MySqlConnection dbConnection = await DBHelper.GetOpenCon())
            {
                using (MySqlTransaction trans = dbConnection.BeginTransaction())
                {
                    ...
                    await IdAmpSample.Delete(0, id, dbConnection, trans);
        ...
        public static async Task<IEnumerable<IdAmpModel>> GetList(string number, string user_id, string id_days, string id_daye,
            string page_size, string page_index)
        {
            ...
            return await DBHelper.GetList<IdAmpModel, long>(TABLE, "*", "id", fdict, "and",
        ...
        public static async Task<IdAmpModel> GetOne(ulong id)
        {
            return await DBHelper.GetOne<IdAmpModel>(id.ToString());
        }
        public static async Task DeleteRedis(ulong id)
        {
            if (id <= 0) return;
            await Redis.Delete<IdAmpModel>(id.ToString());
        }
    }
}
