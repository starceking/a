...
namespace DBOper
{
    public static class User
    {
        ...
        public static async Task<UserModel> Login(string police_no, string login_pwd)
        {
            ...
            //委托用户可以进任意网站、实验室用户只能进自己的网站
            if (user != null && user.login_pwd.Equals(Md5.GetMd5(login_pwd)) &&
              (DictSettings.DeptIsCg(user.dept_no) || user.dept_no.Equals(DictSettings.LAB_NO)))
                return user;
            return null;
        }
        /// <summary>
            ...
            string name, string id_card_no, string auth_ids, int user_id)
        {
            if (string.IsNullOrWhiteSpace(dept_no) || string.IsNullOrWhiteSpace(police_no) ||
                string.IsNullOrWhiteSpace(login_pwd) || user_id <= 0)
                return "参数不全";
            ...
            //委托用户都可以添加、实验室用户只有本实验室可以添加
            int cg_flag = 0;
            if (DictSettings.DeptIsCg(dept_no))
            {
                cg_flag = 1;
                auth_ids = DictSettings.CgUserAuths;
            }
            else if (!user.dept_no.Equals(dept_no)) return "权限异常";
            ...
            dict.Add("cg_flag", cg_flag.ToString());
            ...
            string name, string id_card_no, string auth_ids, int user_id)
        {
            ...
            if (DictSettings.DeptIsCg(user.dept_no))
            {
                ...
                {
                    if (!user.dept_no.Equals(userm.dept_no)) return "权限异常";
                    else if (!userm.dept_no.Equals(dept_no)) return "权限异常";
                }
            }
            else
            {
                if (DictSettings.DeptIsCg(dept_no))
                {
                    if (user.dept_no.Equals(userm.dept_no))
                    {
                        cg_flag = 1;
                        auth_ids = DictSettings.CgUserAuths;
                    }
        ...
        public static async Task<string> ResetPwd(int id, string login_pwd, int user_id)
        {
            ...
            if (userm == null) return "读取不到USERM";
            //委托用户都可以修改、实验室用户只有本实验室可以修改
            if (!DictSettings.DeptIsCg(user.dept_no) && !user.dept_no.Equals(userm.dept_no))
                return "权限异常";
        ...
        public static async Task<UserModel> GetOne(int id, string police_no)
        {
            ...
            else if (!string.IsNullOrWhiteSpace(police_no))
            {
                ...
                user = await DBHelper.GetOne<UserModel, int>(TABLE, fdict, "and");
            }
            return user;
        }
        public static async Task<IEnumerable<UserModel>> GetList(string dept_no, string police_no, string name,
            string cg_flag, string page_size, string page_index)
        {
            if (!DictSettings.DeptIsSub(DictSettings.LAB_NO, dept_no)) return new List<UserModel>();
            ...
            if (!string.IsNullOrWhiteSpace(cg_flag)) fdict.Add("cg_flag", cg_flag);
            return await DBHelper.GetList<UserModel, int>(TABLE, "*", "id desc", fdict, "and",
                Convert.ToInt32(page_size), Convert.ToInt32(page_index));
        }
        public static async Task<long> GetCount(string dept_no, string police_no, string name, string cg_flag)
        {
            if (!DictSettings.DeptIsSub(DictSettings.LAB_NO, dept_no)) return 0;
