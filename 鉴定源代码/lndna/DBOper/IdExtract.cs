...
namespace DBOper
{
    public static class IdExtract
    {
        public const string TABLE = "id_extract";

        public static async Task<string> Insert(string number, string name, int user_id, string shelf_type,
            DateTime id_day, string machine, string remark, string sampleIds)
        {
                ...
                using (MySqlTransaction trans = dbConnection.BeginTransaction())
                {
                    ...
                    foreach (string sid in sampleIds.Split(','))
                    {
                        ...
                        if (ulong.TryParse(sid, out sample_id))
                            await IdExtractSample.Insert(newId, sample_id, user_id, dbConnection, trans);
                    }
        ...
        public static async Task<string> Update(ulong id, string number, string name, int user_id, string shelf_type,
            DateTime id_day, string machine, string remark, int oper_id)
        {
            if (user_id <= 0 || string.IsNullOrWhiteSpace(number) || string.IsNullOrWhiteSpace(name) ||
                string.IsNullOrWhiteSpace(shelf_type) || id_day == null)
                return "参数不全";
            IdExtractModel ipm = await GetOne(id);
        ...
        public static async Task<string> Delete(ulong id, int user_id)
        {
            ...
            using (MySqlConnection dbConnection = await DBHelper.GetOpenCon())
            {
                using (MySqlTransaction trans = dbConnection.BeginTransaction())
                {
                    ...
                    await IdExtractSample.Delete(0, id, dbConnection, trans);
        ...
        public static async Task<IEnumerable<IdExtractModel>> GetList(string number, string user_id, string id_days, string id_daye,
            string page_size, string page_index)
        {
            ...
            return await DBHelper.GetList<IdExtractModel, long>(TABLE, "*", "id", fdict, "and",
        ...
        public static async Task<IdExtractModel> GetOne(ulong id)
        {
            return await DBHelper.GetOne<IdExtractModel>(id.ToString());
        }
        public static async Task DeleteRedis(ulong id)
        {
            if (id <= 0) return;
            await Redis.Delete<IdExtractModel>(id.ToString());
        }
    }
}
