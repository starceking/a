...
{
    public class CASE_PERSONNEL_SAMPLEOracleDAL
    {
        public static void AddWithTransaction(string ID, string CASE_ID, string RELATION_WITH_CASE, string PERSONNEL_NAME, string SAMPLE_TYPE, string GENDER,
            ...
            string PERSONNEL_NO, string EXAMINE_USER1, string reception_man_id, OracleConnection dbConnection, OracleTransaction trans)
        {
            ...
            strSql.Append("insert into gdna.CASE_PERSONNEL_SAMPLE(");
            strSql.Append("ID, CASE_ID,RELATION_WITH_CASE,PERSONNEL_NAME,SAMPLE_TYPE, GENDER,PERSONNEL_TYPE,BIRTH_DATE,NATIONALITY,DISTRICT,ID_CARD_NO,EDUCATION_LEVEL,IDENTITY,NATIVE_PLACE_ADDR,RESIDENCE_ADDR,SAMPLE_PACKAGING,SAMPLE_DESCRIPTION,REMARK,SAMPLE_LAB_NO,PERSONNEL_NO,EXAMINE_USER1,STORE_FLAG,STR_FLAG,LAB_ID,INIT_SERVER_NO,CREATE_USER,CREATE_DATETIME)");
            strSql.Append(" values (");
            strSql.Append(":ID,:CASE_ID,:RELATION_WITH_CASE,:PERSONNEL_NAME,:SAMPLE_TYPE,: GENDER,:PERSONNEL_TYPE,to_date(:BIRTH_DATE,'yyyy-mm-dd'),:NATIONALITY,:DISTRICT,:ID_CARD_NO,:EDUCATION_LEVEL,:IDENTITY,:NATIVE_PLACE_ADDR,:RESIDENCE_ADDR,:SAMPLE_PACKAGING,:SAMPLE_DESCRIPTION,:REMARK,:SAMPLE_LAB_NO,:PERSONNEL_NO,:EXAMINE_USER1,:STORE_FLAG,:STR_FLAG,:LAB_ID,:INIT_SERVER_NO,:CREATE_USER,sysdate)");
            OracleParameter[] parameters = {
					new OracleParameter(":ID",Helper.GetDBValue(ID)),
					new OracleParameter(":CASE_ID",Helper.GetDBValue(CASE_ID)),
					new OracleParameter(":RELATION_WITH_CASE",Helper.GetDBValue(RELATION_WITH_CASE)),
            ...
            LAW_CASEOracleDAL.InsertSample_Examination(Helper.GenerateID(), ID, reception_man_id,
                              EXAMINE_USER1, EXAMINE_USER1, Helper.GetSampleCategoryByRelationWithCase(RELATION_WITH_CASE).ToString(), dbConnection, trans);
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update GDNA.CASE_PERSONNEL_SAMPLE set ");
