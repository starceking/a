...
public partial class OpenWord_OpenWord : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (caption == null || caption.Length == 0) caption = "文档查看";
        //打开人的姓名
        string opener = Request.QueryString["opener"];
        if (opener == null || opener.Length == 0) opener = "匿名" + " " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
        else opener = HttpUtility.UrlDecode(opener) + " " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
        //word模板的硬盘地址，新建模式必须传-----------------------------------
        ...
        //对应Flex的escape加密函数进行解密
        name = HttpUtility.UrlDecode(name);
        path = HttpUtility.UrlDecode(path);

        name = GetFile(path, name);
        //打开的模式：只读、新建、编辑
        ...
        {
            // 设置保存文件页面
            string sss = "?path=" + HttpUtility.UrlEncode(path);
            PageOfficeCtrl1.SaveFilePage = "SaveFile.aspx" + sss;
        ...
        //打开文件
        PageOfficeCtrl1.WebOpen(name, omt, opener);
    }
    #region 此处填充word内容，该处代码根据实际情况编写
    /// <summary>
    /// 填充书签
    ...
    {
        return path + name;
        //return ConfigurationManager.AppSettings["WebPath"] + @"鉴定档案\" + caseId + @"\文书记录\" + name;
        //return @"C:\dev src\pageoffice\OpenWord\doc\" + name + ".doc";
    }
    #endregion
}
